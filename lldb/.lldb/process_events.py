...
'''
    ...
    processGetExitStatus = 0

    if target:

        # .lldbinit defines custom commands like udsdump
        try:
            run_commands( command_interpreter, [ 'command source ~/.lldbinit' ] )
        except:
            print 'Can\'t load .lldbinit. Custom commands won\'t be available.'
            return
            ...
            if process and process.GetProcessID() != lldb.LLDB_INVALID_PROCESS_ID:
                ...
                while not done:
                    ...
                    if listener.WaitForEvent(options.event_timeout, event):
                        if lldb.SBProcess.EventIsProcessEvent(event):
                            ...
                            if state == lldb.eStateInvalid:
                                ...
                                if state == lldb.eStateStopped:
                                    ...
                                    stop_idx += 1
                                    # Yury: my code/changes begin.
                                    #print_threads(process, options)
                                    thread = process.GetThreadAtIndex(0)
                                    if thread.IsValid():
                                        if thread.GetStopReason() == lldb.eStopReasonBreakpoint:
                                            run_commands( command_interpreter, options.stop_commands)
                                        elif thread.GetStopReason() == lldb.eStopReasonException or thread.GetStopReason() == lldb.eStopReasonSignal:
                                    ...
                                    processGetExitStatus = process.GetExitStatus( )
                                    run_commands(
                                ...
                                # Now dump the stdout and stderr
                                while True:
                                    process_stdout = process.GetSTDOUT(1024)
                                    if len( process_stdout ):
                                        print "%s" % (process_stdout),
                                    else:
                                        break
                                while True:
                                    process_stderr = process.GetSTDERR(1024)
                                    if len( process_stderr ):
                                        print "%s" % (process_stderr),
                                    else:
                                        break
                        else:
                            print 'event = %s' % (event)
                        ...
                        process.Stop( ) # Stop the process so that it won't change while being dumped
                        run_commands( command_interpreter, options.crash_commands)
                        processGetExitStatus = 2
